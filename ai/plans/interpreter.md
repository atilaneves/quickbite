...
- An interface method taking a parameter, called through an interface-typed
  ...
  `Speaker.score()`). Found via `tests/example.d` (`example-corpus.md`); no
...
- A zero-arg interface method called through an interface-typed variable
  ...
  the wrong value, with no diagnostic. Found via `tests/example.d`
  (`example-corpus.md`); no matrix fixture pins it yet.
- Nested-AA auto-vivification through plain index assignment
  ...
  for the compound-`+=` sibling shape). Found via `tests/example.d`
  (`example-corpus.md`); no matrix fixture pins the combined-content
  trigger yet.
