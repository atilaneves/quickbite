...
integer literals, float literals, and simple `AddExp`, `MinExp`, `MulExp`, and
`DivExp`, `CastExp`, and unary `NegExp` expressions into a single entry block,
and `vm.d` executes that block directly before converting the returned IR value
to `quickbite.lang.Value` at the backend boundary. Scalar IR types are
D-aware enough to preserve signedness, character-ness, and float width at the
backend boundary without storing `quickbite.lang.Value` in IR instructions or
VM registers.
...
likely expression-only candidate in `tests/ut/backends/lang/eval.d` is
`stringLiteralIsArray`, but verify the current checkout before editing because
backend progress notes can go stale. `multiCell` and the intervening scalar
cast/floating tests appear earlier in the file, but they require
statement/local-state support rather than just expression lowering.

### Next Slice Handoff

Start with `tests/ut/backends/lang/eval.d`. Verify that
`stringLiteralIsArray` still excludes `IR`; if it already includes `IR`, treat
this handoff as stale and choose the next smallest eval behavior that still
excludes `IR`.

If `stringLiteralIsArray` still excludes `IR`, the next TDD slice is:

1. Promote only the existing `stringLiteralIsArray` backend matrix to include
   `IR`.
2. Run `ut.backends.lang.eval.stringLiteralIsArray.IR` and confirm it is red.
3. Inspect the DMD AST that reaches `quickbite.backends.ir.compiler`; the
   lowered IR must reflect the AST shape actually present after semantic
   analysis.
4. Add the smallest production support for the unsupported string literal
   shape that the promoted matrix requires.
...
- `Type` enum: D-aware scalar categories for the currently promoted scalar
  eval tests, plus `ptr`. The IR still does not represent D's full type
  hierarchy, but it preserves enough scalar metadata for casts and final result
  conversion without runtime `quickbite.lang.Value` dispatch. `ptr` represents
  D pointer values, not mutable places. Add a separate `Place` or `PlaceId`
  representation when `Alloca`, `Load`, and `Store` are promoted.
