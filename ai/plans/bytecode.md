...
2. **One place resolver.** Lvalue addressing is enumerated per shape: the
   ...
   than independently reconstructing a module offset. Next, make
   `referenceOffsetOrNull` consume a resolved place and its write-back rule
   directly. A new language-surface shape extends the resolver, never adds a
   sibling emitter.
