...
public abstract class ConfigException : Exception
{
    ...
    ***************************************************************************/
        ...
        const scope @safe
    {
        ...
        char[20] buffer = void;
        // Copy value-type fields to locals so @trusted lambdas don't capture scope this.
        const posLine = this.yamlPosition.line;
        const posCol = this.yamlPosition.column;
        ...
        // dip1000: unsignedToTempString takes buf by non-scope ref; safe because buffer outlives sink.
        sink((() @trusted => unsignedToTempString(posLine, buffer))());
        if (useColors) sink(Reset);
        sink(":");
        if (useColors) sink(Cyan);
        sink((() @trusted => unsignedToTempString(posCol, buffer))());
        if (useColors) sink(Reset);
        sink("): ");
...
package final class TypeConfigException : ConfigException
{
        ...
        const scope @safe
    {
        ...
        // dip1000: scope sink passed to formattedWrite template becomes @system;
        // the calls are safe because immutable string fields don't escape.
        if (useColors)
            () @trusted { formattedWrite(sink, fmt, this.expected.paint(Green), this.actual.paint(Red)); }();
        else
            () @trusted { formattedWrite(sink, fmt, this.expected, this.actual); }();
    }
}
...
package final class DurationTypeConfigException : ConfigException
{
        ...
        const scope @safe
    {
        ...
        // dip1000: scope sink passed to formattedWrite template becomes @system
        if (useColors)
            () @trusted { formattedWrite(sink, fmt, this.actual.paint(Red),
                           this.DurationSuffixes.map!(s => s.paint(Green))); }();
        else
            () @trusted { formattedWrite(sink, fmt, this.actual, this.DurationSuffixes); }();
    }
}
...
public class UnknownKeyConfigException : ConfigException
{
        ...
        const scope @safe
    {
        ...
        if (hasMatch)
        {
            ...
            if (useColors)
                () @trusted { formattedWrite(sink, fmt, matches.map!(f => f.paint(Green))); }();
            else
                () @trusted { formattedWrite(sink, fmt, matches); }();
        }
        else
        {
            ...
            if (useColors)
                () @trusted { formattedWrite(sink, fmt, this.fieldNames.length.paint(Yellow),
                               this.fieldNames.map!(f => f.paint(Green))); }();
            else
                () @trusted { formattedWrite(sink, fmt, this.fieldNames.length, this.fieldNames); }();
        }
    }
}
